...
function master_test() {
  ...
  grep 'INSPECT_TRUE' >/dev/null <<< "$( image inspect "test_termcalc_base_$OS":latest && echo INSPECT_TRUE || echo INSPECT_FALSE)"
  if [[ "$?" == "0" ]]; then
    echo  "  > Base image for $OS already exists, skipping build."
  else
    echo  "  > Building base image... "
     build  -t  "test_termcalc_base_$OS"  --build-arg OS="$OS"  --build-arg TERM="$TERM"  --build-arg COMPILER="$COMPILER"  --build-arg CPPSTANDARD="$CPPSTANDARD"  $EXTRA_DOCKER_BUILD_ARGS -f  "CompatibilityTests/Dockerfile.base_$OS" .
    if [[  "$?"  != 0 ]]; then
  ...
  opposite_compiler="$(if [ "${COMPILER}" = "clang++" ]; then echo g++; else echo clang++; fi)"
   build -f "CompatibilityTests/Dockerfile.$OS" -t "test_termcalc_build_$(tr '[:upper:]' '[:lower:]' <<< "$OS")" . --build-arg OPPOSITE_COMPILER="$opposite_compiler" --build-arg TERM="$TERM" --build-arg COMPILER="$COMPILER" --build-arg CPPSTANDARD="$CPPSTANDARD" $EXTRA_DOCKER_BUILD_ARGS &>"/tmp/termcalc_docker_build_logs/$dirId.log" && echo "IMAGE BUILT SUCCESSFULLY!"
  if [[ $? -ne 0 ]]; then
...
export -f master_test
 ...
 run -i --rm -v ./:/var/pict:Z pict CompatibilityTests/pictmodel.txt 2>&1 | tail +2 | while IFS= read -r line; do
